...
제목 줄 오른쪽의 **⚙** 를 누르면 열립니다. 「창 닫기」와 「창 전환」 두 칸이 있습니다.

![창 닫기 설정](images/tray_setting.png)

**창 닫기** — 오른쪽 위 **✕** 를 눌렀을 때 무엇을 할지 고릅니다. 기본은 **앱 종료**이고, 게임 세션과
캡처도 함께 꺼집니다. **트레이로 숨기기**를 고르면 창만 사라지고 수집과 오버레이는 계속 돕니다.
숨긴 뒤에는 알림 영역의 트레이 아이콘을 누르면 다시 열리고, 우클릭 「종료」로 끕니다.

![창 전환 설정](images/client-focus.png)

**창 전환** — 단축키로 게임 창을 앞으로 가져옵니다. 누를 때마다 다음 창으로 넘어가는 키와,
1·2·3번 창으로 곧장 가는 번호 키를 따로 둘 수 있습니다. **Ctrl·Alt 조합만** 쓸 수 있으며, 게임에서
쓰는 키(예: Alt+1)는 게임 동작과 창 전환이 함께 일어나니 피해주세요. 아래 「지금 넘겨보기」로 바로
시험해 볼 수 있고, 「전환 차례」에 지금 떠 있는 게임이 순서대로 보입니다.
...
관리자로 실행되는 앱이라, 다른 사용자가 파일을 바꿔치기할 수 없도록 `Program Files` 에 설치됩니다.
...
단, 설치본에 함께 담긴 **제3자 구성 요소**(패킷을 읽는 Wireshark 의 dumpcap 과 그 라이브러리)는
위 제한을 받지 않고 **각자의 라이선스**(GPLv2·LGPL 등)를 따릅니다. 목록과 라이선스 전문,
소스 입수 경로는 [**제3자 라이선스 고지**](THIRD-PARTY-LICENSES.md)에 있으며, 설치 폴더에도
같은 문서와 `licenses/` 폴더가 함께 들어 있습니다.

문의·소스 요청: ghj667067@gmail.com
